...
struct FabBarRepresentable<Value: Hashable>: UIViewRepresentable {
	...
	private func configureSegmentContent(
		on control: TabBarSegmentedControl,
		context: Context
	) {
	...
	private func makeContentView(
		...
		case .system(let name):
			let symbolName = filled ? name + ".fill" : name
		...
		case .asset(let name, let bundle, let rendering):
			let uiRenderingMode: UIImage.RenderingMode = rendering == .template ? .alwaysTemplate : .alwaysOriginal
			
			// .original images (e.g. profile photos) don't have a -duotone variant —
			// use the same asset for both base and accent, ring handles the selected state.
			let finalName = uiRenderingMode == .alwaysOriginal ? name : (filled ? name + "-duotone" : name)
			let showRing = filled && uiRenderingMode == .alwaysOriginal
			
			return TabItemContentView(
				title: tab.title,
				imageName: finalName,
				imageBundle: bundle,
				renderingMode: uiRenderingMode,
				showRing: showRing,
			)
		}
	}
